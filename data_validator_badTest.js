...
# =============================================================================
# n8n CODE NODE EXECUTION - UPDATED FOR SEQUENTIAL FLOW
# =============================================================================
...
if 'items' in globals():
    try:
        ...
        if validation_result['status'] == 'FAIL':
            ...
            # Return validation report with FAIL status
            # Validation_Gate Switch will route this to error handling
            return [{
                'json': {
            ...
            # IMPORTANT: Return original items so fact_contract can use them
            # Add validation metadata to first item only
            output_items = []
            for idx, item in enumerate(items):
                item_json = item.get('json', {}).copy() if isinstance(item, dict) else {}
                if idx == 0:
                    # Add validation info to first item
                    item_json['_validation'] = {
                        'status': 'WARNING',
                        'warnings': validation_result.get('warnings', []),
                        'passed': True
                    }
                output_items.append({'json': item_json})
